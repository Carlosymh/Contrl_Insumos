                <div class="selec_filter">
                 <form action="crud.php" method="POST"  class="form">
                   <select name="filtro" id="filtro" class="form-filter" >
                       ...
                       <option value="responsable">Pallets Recibidos</option>
                       <option value="paquetera">Pallets en buen estado</option>
                       <option value="no_gia">Pallets en mal estado</option>
                       <option value="no_paquete">Gaylords Totales Recibidos</option>
                       <option value="tipo_paquete">Gaylords en buen estado</option>
                       <option value="estatus">Gaylords en mal estado</option>
                       <option value="razon_rechazo">Cajas</option>
                   </select>
                   ...
                   if(empty($_POST['valor'])){
                    echo "<a href='archivos/cvs_r_fc.php' class='descarga_doc'><i class='fas fa-x2 fa-file-download'></i></a>" ;
                   }else if(empty($_POST['fechaa']) && empty($_POST['fechad'])){
                    $filtro=$_POST['filtro'];
                    $valor=$_POST['valor'];
                       echo '<a href="archivos/cvs_r_fc.php?filtro='.$filtro.'&valor='.$valor.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i></a>';
                      }else if(empty($_POST['fechaa'])){
                        $filtro=$_POST['filtro'];
                        $valor=$_POST['valor'];
                        $fecha1=$_POST['fechad'];
                           echo '<a href="archivos/cvs_r_fc.php?filtro='.$filtro.'&valor='.$valor.'&fechad='.$fecha1.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i></a>';
                       }else {
                    $filtro=$_POST['filtro'];
                    $valor=$_POST['valor'];
                    $fecha1=$_POST['fechad'];
                    $fecha2=$_POST['fechaa'];
                       echo '<a href="archivos/cvs_r_fc.php?filtro='.$filtro.'&valor='.$valor.'&fechad='.$fecha1.'&fechaa='.$fecha2.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i></a>';
                   }
                  ?></div>
                <table >
                 <tr>
              ...
              $totalP=round(($totalR/$limit),2)+1;
                   ...
                   if(empty($_POST['valor'])){
                $sql2="SELECT * FROM `recibo_fc` LIMIT $offset, $limit" ;
              $resultado=mysqli_query($enlace,$sql2);
                    }else if(empty($_POST['fechaa']) && empty($_POST['fechad'])  ){
                $filtro=$_POST['filtro'];
                $valor=$_POST['valor'];
                  $sql2="SELECT * FROM `recibo_fc` WHERE $filtro LIKE '%$valor%' LIMIT $offset, $limit ";
              $resultado=mysqli_query($enlace,$sql2);
                ...
                $valor=$_POST['valor'];
                $fecha1=$_POST['fechad'];
                  $sql2="SELECT * FROM `recibo_fc` WHERE $filtro LIKE '%$valor%' AND dia = '$fecha1' LIMIT $offset, $limit ";
              $resultado=mysqli_query($enlace,$sql2);
                    }else{
                $filtro=$_POST['filtro'];
                $valor=$_POST['valor'];
                $fecha1=$_POST['fechad'];
                $fecha2=$_POST['fechaa'];
                  $sql2="SELECT * FROM `recibo_fc` WHERE $filtro LIKE '%$valor%' AND dia BETWEEN '$fecha1' AND '$fecha2' LIMIT $offset, $limit ";
              $resultado=mysqli_query($enlace,$sql2);
                  ...
                  while($mostrar=mysqli_fetch_array($resultado)){
                      ...
                      echo "<tr><td>".$mostrar['id_recibo']."</td>";
                      echo "<td>".$mostrar['Fulfillment']."</td>";
                      echo "<td>".$mostrar['responsable']."</td>";
                      echo "<td>".$mostrar['paquetera']."</td>";
                      echo "<td>".$mostrar['no_gia']."</td>";
                      echo "<td>".$mostrar['no_paquete']."</td>";
                      echo "<td>".$mostrar['tipo_paquete']."</td>";
                      echo "<td>".$mostrar['estatus']."</td>";
                      echo "<td>".$mostrar['razon_rechazo']."</td>";
