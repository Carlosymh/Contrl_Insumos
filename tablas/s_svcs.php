                 <div class="selec_filter">
                  <form action="crud.php" method="POST"  class="form">
                   ...
                   if(empty($_POST['valor'])){
                               echo "<a href='archivos/cvs_s_svcs.php' class='descarga_doc'><i class='fas fa-x2 fa-file-download'></i></a>" ;
                           }else if(empty($_POST['fechaa']) && empty($_POST['fechad'])  ){
                               $filtro=$_POST['filtro'];
                               $valor=$_POST['valor'];
                                  echo '<a href="archivos/cvs_s_svcs.php?filtro='.$filtro.'&valor='.$valor.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i></a>';
                                }else if(empty($_POST['fechaa'])){
                                  $filtro=$_POST['filtro'];
                                  $valor=$_POST['valor'];
                                     echo '<a href="archivos/cvs_s_svcs.php?filtro='.$filtro.'&valor='.$valor.'&fechad='.$fecha1.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i></a>';
                             }else{
                               $filtro=$_POST['filtro'];
                               $valor=$_POST['valor'];
                               $fecha1=$_POST['fechad'];
                               $fecha2=$_POST['fechaa'];
                                  echo '<a href="archivos/cvs_s_svcs.php?filtro='.$filtro.'&valor='.$valor.'&fechad='.$fecha1.'&fechaa='.$fecha2.'" class="descarga_doc"><i class="fas fa-x2 fa-file-download"></i> </a>';
                           }
                           ?> 
                           </div>
                             <table >
                           ...
                           if(empty($_POST['valor'])){
                               $sql2="SELECT * FROM `salida_svcs` LIMIT $offset, $limit" ;
                             $resultado=mysqli_query($enlace,$sql2);
                           }else if(empty($_POST['fechaa']) && empty($_POST['fechad'])  ){
                               $filtro=$_POST['filtro'];
                               $valor=$_POST['valor'];
                                 $sql2="SELECT * FROM `salida_svcs` WHERE $filtro LIKE '%$valor%' LIMIT $offset, $limit ";
                             $resultado=mysqli_query($enlace,$sql2);
                               ...
                               $valor=$_POST['valor'];
                               $fecha1=$_POST['fechad'];
                                 $sql2="SELECT * FROM `salida_svcs` WHERE $filtro LIKE '%$valor%' AND Fecha_Creación = '$fecha1' LIMIT $offset, $limit ";
                             $resultado=mysqli_query($enlace,$sql2);
                           }else{
                               $filtro=$_POST['filtro'];
                               $valor=$_POST['valor'];
                               $fechad=$_POST['fechad'];
                               $fechaa=$_POST['fechaa'];
                                 $sql2="SELECT * FROM `salida_svcs` WHERE $filtro LIKE '%$valor%' AND Fecha_Creación BETWEEN '$fechad' AND '$fechaa' LIMIT $offset, $limit ";
                             $resultado=mysqli_query($enlace,$sql2);
